...
%N:初始化群体个体数目
%eps:设置精度
%DS:每隔DS次循环就检查最优个体是否变化
%replaceP:粒子的概率大于repalceP将被免疫替换
%minD:粒子间的最小距离
%Psum:个体的最佳和
range=100
count=0
...
for t=1:MaxDT
    ...
    %if Foxhole(pg,D)<eps                %如果满足精度要求则跳出循环
    %    break
    %end
    %%%%%%开始进行免疫%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if t>DS
        if mod(t,DS)==0&&(Pbest(t-DS+1)-Pbest(t))<1e-020
%如果连续DS代数，群体最优没有明显变化，则进行免疫，在函数测试的过程中发现，经过
%一定代数的的更新，个体最优不完全相等，但变化非常小
            for i=1:N                      %先计算出个体最优和
                Psum=Psum+p(i)
            end
            for i=1:N                      %免疫程序
                for j=1:N                  %计算出每个个体与个体i的距离
